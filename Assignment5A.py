...
def reset(score):
    ...
    return collide, ticker, ticker2, asteroids, spawnRate, timer, counter
...
while run:
    ...
    for key, (dirX, dirY) in movement.items():
        if keys[key]:
            ...
            if 0 <= new_rect.x <= width-smallRect.width and 0 <= new_rect.y <= height-smallRect.height:
                smallRect = new_rect
    if ticker == 0: #make an asteroid every so often and change the spawn rate by -0.5 seconds every 5 spawns until it reaches 1 second
        ...
        if counter == 5:
            spawnRate -= 30 if spawnRate > 60 else 0
            counter = 0
    if ticker2 == 60:
        timer += 1
    ...
    if not collide:
        ...
        screen.blit(surf, (0, 0))
    if collide:
        collide, ticker, ticker2, asteroids, spawnRate, timer, counter = reset(timer)
    pygame.display.flip()
    clock.tick(60)
